...
const BaseDashboard = ({ children, navItems, title, userInfo, hideMenu, initialActiveView = 'dashboard' }) => {
  ...
  const handleNavigation = (view) => {
    // Handle the Patient View navigation differently
    if (view === 'patients') {
      // First check if the patientId is available in localStorage
      const patientId = localStorage.getItem('patientId');
      if (patientId) {
        // Navigate to the patient view with the ID
        navigate(`/doctor/patient/${patientId}`);
      } else {
        // If no patient ID available, show an alert
        alert('No patient ID found. Please verify a patient first.');
      }
    } else {
  ...
  // Define navigation items, conditionally including Patient View for non-patients
  const navigationItems = [
    { id: 'dashboard', label: 'Dashboard', icon: FiHome },
    { id: 'prescriptions', label: 'Prescriptions', icon: FiFileText },
    ...(isPatient ? [] : [{ id: 'patients', label: 'Patient View', icon: FiUsers }]),
  ...
  const renderSettingsModal = () => {
    ...
    <div className="h-screen overflow-hidden bg-gray-50 dark:bg-gray-900">
      {/* Top Header */}      <header className="fixed top-0 left-0 right-0 h-14 sm:h-16 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between px-2 sm:px-4 md:px-6 z-20">
      ...
      <div className="flex pt-14 sm:pt-16 h-full">        {/* Mobile sidebar overlay */}
        {showMobileSidebar && (
          <div 
        ...
        >          <div className={`mb-3 sm:mb-4 md:mb-6 flex ${isSidebarCollapsed ? 'md:justify-center' : 'justify-between'} items-center`}>
          ...
          </div><nav className="space-y-1 flex-1">
          ...
          </nav>          <div className="mt-auto pt-3 sm:pt-4 border-t border-gray-200 dark:border-gray-700">
            <button
              onClick={() => {
