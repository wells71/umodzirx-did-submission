/**
 * Enhanced Blockchain Integration Test Script
 * 
 * This script tests the full lifecycle of prescriptions with multiple scenarios,
 * with enhanced waiting mechanisms for blockchain transactions and better error handling.
 * It covers all key user responsibilities with more retries and longer wait times to 
 * account for blockchain commit latencies.
 * 
 ...
 * 
 * Run with: node scripts/enhanced-test.js
 */
...
// Helper for checking API response status with enhanced error details
const checkResponseStatus = (response, operationName) => {
  // Log raw response for debugging
  console.log(`DEBUG: ${operationName} raw response:`, JSON.stringify(response.data).substring(0, 200) + '...');
  
  if (!response.data || !response.data.success) {
    console.log(`WARNING: ${operationName} failed with response:`, JSON.stringify(response.data));
    
    if (typeof response.data === 'string' && response.data.includes('Error:')) {
      throw new Error(`${operationName} failed with blockchain error: ${response.data}`);
    }
